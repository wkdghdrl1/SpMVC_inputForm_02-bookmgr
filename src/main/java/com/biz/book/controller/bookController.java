...
public class bookController {
	 ...
	 * VO 클래스를 만들어서 @ModerAttribute로 받는다.
	 * 
	 * spring에서는
	 * form에서 보내는 데이터를 받을 때
	 * form의  input box에 설정된 name 속성 값과 일치하는
	 * 필드(맴버변수)들을 선언하고, getter, setter, 생성자가 있는
	 * VO를 만들어서 @ModelAttribute 매개변수로 사용하면
	 * 
	 * 자옫으로 해당 클래스를 객체로 선언하고
	 * 각 필드에 값을 setting해서 사용할 수 있도록 구성한다.
	 * 
	 * input box에 입력되는 모든 값은 text(--String)이다.
	 * 매개변수로 값을 받을 때 String이 아닌 다른 type으로 받으면
	 * String to type의 형 변환을 시도한다.
	 * 예를 들어 VO 의  b_price가 int 형이면
	 * 실제로는
	 * setB_price(Integer.valueof(b_price))형식의 코드가 실행될 것이다.
	 * 이 때 b_price에 입력한 값이 숫자로 변환시키는데 문제가 있을경우
	 * (Exception이 발생할 경우)
	 * 
	 * 서버는 브라우저에 400의 http status(상태) 코드를 보낸다.
	 * 
	 * Interger.valuseof("AAA")
	 * Interger.valuseof("123AAA")
	 * Interger.valuseof("AAA123")
	 * Interger.valuseof("")
	 */
